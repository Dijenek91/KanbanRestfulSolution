...
namespace KanbanIntegrationTests
{
    ...
    internal class TaskControllerIntegrationTests
    {
        ...
        //mostly used for testing the GetById, Update and Delete endpoints,
        //to optimized code readability
        private async Task Seed2TasksInDb()
        {
        ...
        public async Task GetAll_Sort_AscName()
        {
            ...
            Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.OK));
            var taskReponse = await response.Content.ReadFromJsonAsync<PagedResultKanbanTasksResponse<KanbanTaskResponse>>();
            Assert.That(taskReponse.TotalCount, Is.EqualTo(unsortedKanbanTasks.Count));
        ...
        public async Task UpdateFullTask_ReturnsNoContent()
        {
            ...
            //Get all tasks to obtain the id of the task we want to update,
            //since the id is generated by the database and not known beforehand
            ...
            // Update the task with the obtained id
            
            var putResponse = await _client.PutAsJsonAsync($"/api/tasks/{taskIdToUpdate}", request);

            Assert.That(putResponse.StatusCode, Is.EqualTo(HttpStatusCode.NoContent));

            // Get the updated task by id to verify the changes

            var getByIdResponse = await _client.GetAsync($"/api/tasks/{taskIdToUpdate}");
            var getByIdResponseResult = await getByIdResponse.Content.ReadFromJsonAsync<KanbanTaskResponse>();

            Assert.That(getByIdResponse.StatusCode, Is.EqualTo(HttpStatusCode.OK));
            Assert.That(getByIdResponseResult.Name, Is.EqualTo(request.Name));
            Assert.That(getByIdResponseResult.Description, Is.EqualTo(request.Description));
            Assert.That(getByIdResponseResult.PriorityEnum, Is.EqualTo(request.PriorityEnum));
            Assert.That(getByIdResponseResult.Size, Is.EqualTo(request.Size));
            Assert.That(getByIdResponseResult.Status, Is.EqualTo(request.Status));
        }
        ...
        public async Task UpdatePartialTask_ReturnsNoContent()
        {
            ...
            var response = await _client.PatchAsJsonAsync($"/api/tasks/{taskIdToUpdate}", request);

            Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.NoContent));

            // Get the updated task by id to verify the changes
            var getByIdResponse = await _client.GetAsync($"/api/tasks/{taskIdToUpdate}");
            var result = await getByIdResponse.Content.ReadFromJsonAsync<KanbanTaskResponse>();
        ...
        public async Task DeleteTask_ReturnsNoContent()
        {
            ...
            var response = await _client.DeleteAsync($"/api/tasks/{taskIdToUpdate}");

            Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.NoContent));

            // Get the deleted task by id to verify the that it doesnt exist
